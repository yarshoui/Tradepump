...
import { Schema, parse } from "protocol-buffers-schema";
...
export function generate(schema: Schema, options?: Options): string {
  ...
  const pathMap = options.importPathMap ?? {};
  for (const include of schema.imports) {
    if (!pathMap[include]) {
      console.error(`[ERROR] No import path for '${include}', skipping this import`);
      continue;
    }
    // TODO: fix ref
    const content = readFileSync(resolve("protobuf", include), "utf8");
    const importSchema = parse(content);
    const importFilename = pathMap[include];
    const names = importSchema.enums.flatMap(e => [e.name, `encode${e.name}`, `decode${e.name}`]).concat(importSchema.messages.map(m => m.name));
    // Fill map
    importSchema.enums.forEach(e => {
      enums[e.name] = true;
      packableTypes[e.name] = true;
    });
    // const importName = importFilename.replace(/[^a-z_]/gi, "");
    // const importNames = schema.options[importName]?.split(/\s*,\s*/);
    // const importStr = importNames?.length > 0 ? `{ ${importNames.join(", ")} }` : `* as importName`
    lines.push(`import { ${names.join(", ")} } from "${importFilename}";`);
  }
